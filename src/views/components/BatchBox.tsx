...
const BatchBox = ({ isOpen, onClose }: BatchBoxProps) => {
  ...
  const reset = () => {
    ...
    setDescription('');
    setVolume('');
    setBdjModel(PRODUCT_OPTIONS[0].value);
    setIsInsumo(false);
    setPreviewIds([]);
  };
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-zinc-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col overflow-hidden">
          <div className="px-6 py-5 grid grid-cols-2 gap-4 overflow-y-auto">
            ...
            {/* Modelo BDJ — obrigatório quando BDJ selecionado */}
            {isBandeja && (
              <div className="col-span-2">
            ...
            {validCount && (
              <div className="col-span-2">
                ...
                {loadingPreview ? (
                  ...
                  <div className={`border rounded-lg px-3 py-2.5 ${isBandeja ? 'bg-orange-50 border-orange-100' : isInsumo ? 'bg-teal-50 border-teal-100' : 'bg-blue-50 border-blue-100'}`}>
                    <p className={`font-mono text-xs leading-relaxed ${isBandeja ? 'text-orange-700' : isInsumo ? 'text-teal-700' : 'text-blue-700'}`}>
                      {previewIds.slice(0, 10).join('  ·  ')}
                      {previewIds.length > 10 && (
                        <span className={isBandeja ? 'text-orange-400' : isInsumo ? 'text-teal-400' : 'text-blue-400'}>  ·  +{previewIds.length - 10} mais</span>
                      )}
                    </p>
                  </div>
                ) : null}
